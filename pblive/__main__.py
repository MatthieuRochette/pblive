...
def socket_join(session_name):
    ...
    flask_socketio.emit(
        "update",
        flask.render_template("colour_picker.html", session=_session),
        room=flask.request.sid,
    )
    flask_socketio.emit(
        "update_left", render_sidebar(user, _session), room=flask.request.sid
    )
...
def relay_color_change(_user, _data):
    for _, other_user in _data.iterate_users():
        if other_user != _user and other_user.session == _user.session:
            flask_socketio.emit(
                "update_left",
                render_sidebar(other_user, _user.session),
                room=other_user.sid,
            )
            if not other_user.colour:
                flask_socketio.emit(
                    "update",
                    flask.render_template("colour_picker.html", session=_user.session),
                    room=other_user.sid,
                )
...
def socket_register(colour_id, colour_name):
    ...
    flask_socketio.emit(
        "update_left", render_sidebar(user, user.session), room=user.sid
    )
            ...
            if other_user.session == user.session:
                ...
                if isinstance(
                    ...
                    flask_socketio.emit(
                        "update",
                        render_question(
                            other_user, user.session, user.session.question_num
                        ),
                        room=other_user.sid,
                    )
        for _, _admin in data.iterate_admins():
...
def do_goto_question(_session, question_num):
    ...
    for _, other_user in data.iterate_users():
        if other_user.session == _session and other_user.colour:
            ...
            flask_socketio.emit(
                "update_left", render_sidebar(other_user, _session), room=other_user.sid
            )
    for _, _admin in data.iterate_admins():
...
def socket_pass_question():
    user = (
        data.admins[flask.request.sid]
        if flask.request.sid in data.admins
        else data.users[flask.request.sid]
    )

    if isinstance(
        user.session.questions[user.session.question_num], data.RandomQuestion
    ):
        ...
        for _, other_user in data.iterate_users():
            if other_user.session == user.session and other_user.colour:
                flask_socketio.emit(
                    "update",
                    render_question(
                        other_user, other_user.session, other_user.session.question_num
                    ),
                    room=other_user.sid,
                )
                flask_socketio.emit(
        ...
        for _, _admin in data.iterate_admins():
            if _admin.session == user.session:
                flask_socketio.emit(
                    "update",
                    render_question_admin(_admin.session, _admin.session.question_num),
                    room=_admin.sid,
                )
                flask_socketio.emit(
                    "update_left", render_sidebar(_admin, user.session), room=_admin.sid
